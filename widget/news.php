...
class news_index extends WP_Widget
{
    ...
    function form($instance)
    {
        ...
        ?>
        <p style="display:block; width:100%; border-bottom:1px #333333 solid; padding:5px; margin:5px;">
            <strong>模块属性设置</strong></p>
        ...
        <p>
            ...
            <select id="<?php echo $this->get_field_id('w_cat'); ?>"
                    name="<?php echo $this->get_field_name('w_cat'); ?>">
        ...
        <p><label for="<?php echo $this->get_field_id('nnmber2'); ?>"><?php esc_attr_e('显示数量(默认4):'); ?> <input
                        class="widefat" id="<?php echo $this->get_field_id('nnmber2'); ?>"
                        name="<?php echo $this->get_field_name('nnmber2'); ?>" type="text"
                        value="<?php echo $nnmber2; ?>"/></label></p>
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name('showway'); ?>"
                    id="<?php echo $this->get_field_id('showway'); ?>">
                <option value=''>默认上下图文</option>
                <option <?php if ($showway == 1) {
                    echo "selected='selected'";
                }; ?> value='1'>左图右文
                </option>
            </select>
            <em style="padding:3px; background:#FCF3E4; border:solid 1px #F0D8BF; display:block;">你可以选择这个模块的文章显示模式，这种一个模块显示多种样式的方式在<a
                        target="_blank" href="http://www.themepark.com.cn/">WEB主题公园</a>开发的主题被称为“魔术模块”，是一种很方便并很容易学会控制的方法，在<a
                        target="_blank" href="http://www.themepark.com.cn/">WEB主题公园</a>开发的主题中很常见</em>
        </p>
        ...
        <p>
            ...
            <select name="<?php echo $this->get_field_name('my_b_images'); ?>"
                    id="<?php echo $this->get_field_id('my_b_images'); ?>">
                <option value=''>不悬浮</option>
                <option <?php if ($my_b_images == 1) {
                    echo "selected='selected'";
                }; ?> value='1'>悬浮
                </option>
            </select>
        ...
        <p>
            ...
            <textarea style="width:96%" name="<?php echo $this->get_field_name('my_text2'); ?>"
                      id="<?php echo $this->get_field_id('my_text2'); ?>" cols="52"
                      rows="4"><?php echo stripslashes($my_text2); ?></textarea>
    ...
    { // 输出显示在页面上
        ...
        $args           = array('cat' => $w_cat, 'showposts' => $nnmber2);
        ...
        <?php if ($w_cat) { ?>
        <div class="<?php if ($my_b_images) {
            echo 'fixed_mok';
        } ?>">
        <div class="news_tuoch widget " <?php if ($showway) {
            echo 'id="two_show"';
        } ?>>
            ...
            <h2>
                <?php if ($title) { ?> <a
                        href="<?php echo get_category_link($w_cat); ?>"> <?php echo $title; ?></a><?php } ?>
            </h2>
            ...
            <ul>
                ...
                <?php while ($query->have_posts()) :$query->the_post();
                    ...
                    ?>
                    <li>
                        <a class="news_pics" href="<?php echo $links1; ?>"
                           target="_blank"><?php themepark_thumbnails('case'); ?></a>
                        <div class="news_textss">
                            <a href="<?php echo $links1; ?>" target="_blank"
                               title="<?php echo $tit; ?>"><?php echo mb_strimwidth(strip_tags(apply_filters('the_title', $tit)), 0, 30, "...", 'utf-8'); ?></a>
                            <em><?php echo get_the_time('Y/m/d'); ?></em>
                            <?php if ($showway) {
                                echo '<p>' . mb_strimwidth(strip_tags(apply_filters('the_excerp', get_the_excerpt($id))), 0, 50, "...", 'utf-8') . '</p>';
                            } ?>
                        </div>
                    </li>
    ...
    <?php }
        if ($my_text2) { ?>
            <div class="widget">
            <div class="code_s"><?php echo html_entity_decode($my_text2); ?></div></div><?php } ?>
        </div>
        <?php
    }
}
